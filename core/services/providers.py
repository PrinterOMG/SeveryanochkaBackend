...
from core.repositories.providers import get_brand_repository, get_user_repository, get_phone_key_repository, \
    get_country_repository, get_manufacturer_repository, get_category_repository
...
def get_phone_key_service(
        phone_key_repository: Annotated[PhoneKeyRepositoryBase, Depends(get_phone_key_repository)],
        uow: Annotated[UnitOfWorkBase, Depends(get_uow)]
) -> PhoneKeyServiceBase:
    return PhoneKeyService(
        phone_key_repository=phone_key_repository,
        uow=uow
    )
...
def get_auth_service(
    ...
    return AuthService(
        user_service=user_service, phone_key_service=phone_key_service, settings=settings
    )


def get_country_service(
        country_repository: Annotated[CountryRepositoryBase, Depends(get_country_repository)]
) -> CountryServiceBase:
    return CountryService(country_repository=country_repository)


def get_manufacturer_service(
        manufacturer_repository: Annotated[ManufacturerRepositoryBase, Depends(get_manufacturer_repository)],
        uow: Annotated[UnitOfWorkBase, Depends(get_uow)]
...
def get_category_service(
        category_repository: Annotated[CategoryRepositoryBase, Depends(get_category_repository)],
        uow: Annotated[UnitOfWorkBase, Depends(get_uow)]
) -> CategoryServiceBase:
